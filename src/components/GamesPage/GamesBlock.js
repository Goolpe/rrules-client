...
class Games extends Component {
  ...
  render() {
    ...
    const games = this.props.currentTodos || (_.sortBy(this.props.games, ['from']).slice(0, 3));
    const renderTodos = games.map((game, index) =>
      <Link to={`/game/${game._id}`} className='border-0 m-0 p-0 mb-4 btn text-left text_card w-100' key={index}>
        <div className='p-3 bg_card shadow' style={ { whiteSpace: 'normal' } }>
          <div className='row'>
            <div className='col-12'>
              <p className='pb-3 border-bottom'>
                {game.nameGame} {game.gamersInsideId.filter(
                  gamerInside => gamerInside.user === user.player && gamerInside.accept === true)
                  .map(gamer => <span style={ { color: '#4caf50' } } key={index}>| Вы в игре</span>)}
              </p>
            </div>
            ...
            <div className='col-12 col-md-4'>
              <p className='d-flex-wrap'
                style={ { wordWrap: 'break-word' } }
              >
                Всего мест: {game.placeAll - (game.gamersInsideId.filter(
