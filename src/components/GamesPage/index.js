...
class GamesPage extends Component {
  ...
  render() {
    ...
    // sort by date
    let gamesSort = this.props.games;

    this.state.sortByDate ? (gamesSort = _.sortBy(gamesSort, ['from'])) :
    ...
    this.state.selectedOption === 'sortByTypeIRL' && (gamesSort = gamesSort.filter(game =>
      this.state.sortByCityGame.length === 0 ? game.cityGame
        :
        this.state.sortByCityGame.length <= game.cityGame.length && game.cityGame.toLowerCase().includes(
          this.state.sortByCityGame.toLowerCase(),
        )
          ?
          game.cityGame : false,
    ));
        ...
        <PaginationItem key={index}>
          {pageNumbers.length > 1 &&
            <PaginationLink id={number}
              className='shadow bg_card text_card rounded-0 border-0 mr-2'
            ...
            <div className='col-12 col-lg-3 mt-2'>
              <div className='container bg_card shadow text_card pt-3 pb-3'>
                {isAuthenticated && user.status !== 'игрок' &&
                  <Link to='/game-create' className='btn btn-info mb-2 w-100'>
                    Создать игру
                  </Link>
                }
                <ButtonDropdown isOpen={this.state.dropdownOpen} className='w-100 mb-2' toggle={this.toggle}>
                  ...
                  <DropdownMenu className='bg-white'>
                    <DropdownItem onClick={ () => { this.setState({ sortByDate: true }); } }>
                      по дате
                    </DropdownItem>
                    <DropdownItem onClick={ () => { this.setState({ sortByDate: false }); } }>
                      по количеству мест
                    </DropdownItem>
                  </DropdownMenu>
                </ButtonDropdown><br/>
                ...
                <div className='mb-2 text-dark'>
                  ...
                  <div className='text-dark'>
                      ...
                      <input type='radio'
                        ...
                        value='sortByTypeAll'
                        onChange={ () => { this.setState({ selectedOption: 'sortByTypeAll' }); } }
                        checked={this.state.selectedOption === 'sortByTypeAll'}
                        id='radio0'
                      />
                  ...
                  <div className='custom-control custom-radio mb-2'>
                      <input type='radio'
                        ...
                        onChange={ () => { this.setState({ selectedOption: 'sortByTypeOnline' }); } }
                        checked={this.state.selectedOption === 'sortByTypeOnline'}
                        id='radio1'
                      />
                  ...
                  <div className='custom-control custom-radio mb-2'>
                      <input type='radio'
                        className='custom-control-input'
                        value='sortByTypeIRL'
                        onChange={ () => { this.setState({ selectedOption: 'sortByTypeIRL' }); } }
                        checked={this.state.selectedOption === 'sortByTypeIRL'}
                        id='radio2'
                      />
                    ...
                    <div className=' mb-4'>
                      ...
                      <input type='string'
                        value={this.state.sortByCityGame}
                        className="w-100"
