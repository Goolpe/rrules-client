...
class GamePage extends Component {
  ...
  handlePlayer(e) {
    ...
    else if (this.props.game && this.props.game.gamersInsideId.find(msg =>
      msg.user === this.props.auth.user.id && msg.decline === true,
    )) {
      this.notify('Мастер отклонил Ваш запрос!');
    }
    else if (this.props.game && this.props.game.gamersInsideId.find(msg =>
      msg.user === this.props.auth.user.id && msg.decline === false && msg.accept === false,
    )) {
      this.notify('Вы уже отправили запрос!');
    }
    else {
      const gameData = {
        ...
        <section className='text_card container'>
          <div className='row'>
            ...
            <div className='col-auto p-0'>
              {(game && game.name === user.id) &&
                <Link to={`/game-edit/${game._id}`} className='btn btn-outline-info rounded-0 mb-2 mr-2'>
                  Редактировать
                </Link>
              }
              {(game.name === user.id ||
                (game.gamersInsideId && game.gamersInsideId.find(gamer =>
                  gamer.user === user.id && gamer.accept === true,
                )))
                ?
                <Button className='mb-2 btn-outline-secondary' disabled>Вы в игре</Button>
                :
                game.gamersInsideId && game.gamersInsideId.find(
                  gamer => gamer.user === user.id && gamer.decline === true,
                )
                  ?
              ...
              <React.Fragment key={index} >
                ...
                <ul>
                  {this.props.persons.filter(player => game.gamersInsideId
                    ...
                    .map((gamer, index) =>
                      <React.Fragment key={ index }>
                        <Link to={`/@${gamer.name}`} className='text-white' target='_blank' >
                          {gamer.name}
                        </Link>
                        <br/>
                      </React.Fragment>,
