...
class Footer extends Component {
  ...
  render() {
    ...
    const mastersSort = _.sortBy(this.props.persons && this.props.persons.filter(
      person => person.status === 'мастер',
    ), ['rating']).reverse();

    const listItems = articleSort.map(
      (article, index) =>
        <li key={index} className='mt-1 mb-1'>
          <Link className='text-white' to={`/article/${article._id}`}>
            {article.title.length > 25 ? (article.title.slice(0, 25) + '...') : article.title}
          </Link>
        </li>,
    ).slice(0, 7);
    const mastersRating = mastersSort.map(
      (master, index) =>
        <Link key={index} to={`/@${master.name}`} className='text-white'>
          <li>
            <i className='fas fa-star text-warning fa-1x'></i> {master.rating}/5  - {master.name}
          </li>
        </Link>,
    ).slice(0, 8);
    return (
          ...
          <div className='row'>
            <div className='col-12 col-md-8'>
              <p className='text-white'>
                &copy; Copyright 2018 | Random Rules |
                <a className=' text-white'
                  href='https://github.com/Goolpe'
