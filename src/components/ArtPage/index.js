...
class ArtPage extends Component {
  ...
  render() {
    const ArtList = this.props.art.map(
      (img, index) =>
        <figure className="image-block__card m-1" key={ index }>
          ...
          <figcaption className="card__caption"
            onClick={ () => { this.setState({ bigPicture: true, selectedIndex: index }); } }
          >
        ...
        <section className="art__open-picture">
          <div className="row text-center text_card" style={ { height: '100%' } }>
            <div className="col-2 col-md-4 d-flex align-items-center justify-content-center"
              style={ { height: '100%', cursor: 'pointer' } } onClick={this.togglePrev}
            >
              <button className="bg-transparent text-center text_card border-0 p-0">
                <FaChevronLeft size='3em' />
              </button>
            </div>
            <div className="col-8 col-md-4 d-flex align-items-center justify-content-center"
              style={ { height: '100%', cursor: 'pointer' } }
            >
              <figure>
                <img src={ this.props.art[this.state.selectedIndex].photo_604}
                  alt={this.props.art[this.state.selectedIndex].text }
                  className="img-fluid"
                />
            ...
            <div className="col-2 col-md-4 d-flex align-items-center justify-content-center"
              style={ { height: '100%', cursor: 'pointer' } } onClick={this.toggleNext}
            >
              <button className="bg-transparent text-center text_card border-0 p-0">
                <FaChevronRight size="3em" />
              </button>
            </div>
            <button onClick={ () => { this.setState({ bigPicture: false }); } }
              className="fixed-top text-center text-white bg-transparent border-0 p-0"
              style={ { top: '50px', left: '80%' } }
            >
              <FaTimesCircle size='3em' />
            </button>
          </div>
        </section>
        }
